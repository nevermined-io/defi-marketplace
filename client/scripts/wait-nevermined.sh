...
until [ $COMMAND_STATUS -eq 0 ] || [ $RETRY_COUNT -eq 120 ]; do
  ...
  docker cp ${nevermined_keeper_docker_id}:/nevermined-contracts/artifacts/ready $DEST_FOLDER/artifacts/
  COMMAND_STATUS=$?
  sleep 5
...
docker cp ${nevermined_keeper_docker_id}:/nevermined-contracts/artifacts/. $DEST_FOLDER/artifacts/

docker cp ${nevermined_keeper_docker_id}:/nevermined-contracts/circuits/keytransfer.wasm $DEST_FOLDER/circuits/
docker cp ${nevermined_keeper_docker_id}:/nevermined-contracts/circuits/keytransfer.zkey $DEST_FOLDER/circuits/
