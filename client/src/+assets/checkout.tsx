...
export const Checkout: NextPage = () => {
  ...
  const { walletAddress, loginMetamask, switchChainsOrRegisterSupportedChain } =
    MetaMask.useWallet()

  const assets = contextAssets.filter((asset) => basket.includes(asset.id))
  const totalPrice = assets
    .map((asset) => Number(asset.findServiceByType('metadata').attributes.main.price))
        ...
        <UiLayout type="sides" align="start">
          <div className={b('purchases-wrapper')}>
            ...
            {assets.length ? (
              assets
                .map((asset) => ({
                  asset,
                  metadata: asset.findServiceByType('metadata').attributes
                }))
                  ...
                  <div key={asset.id} className={b('table-wrapper')}>
                    <UiLayout className={b('asset')}>
                      ...
                      <Link href={`/asset/${asset.id}`}>
                        <UiText className={`pointer ${b('asset-title')}`} wrapper="h4" type="h4">
                          {metadata.main.name}
                        </UiText>
                      </Link>
          ...
          <div className={b('order-summary')}>
            ...
            <UiLayout justify="end">
              <div className={b('items-selected')}>{basket.length} items</div>
            </UiLayout>
            <UiDivider />
            <UiLayout justify="space-between">
              <UiText type="h4-caps" wrapper="h2">
                TOTAL
              </UiText>
              <div className={b('total-price')}>
                <XuiTokenPrice>{totalPrice}</XuiTokenPrice>
              </div>
            </UiLayout>
            <UiDivider />
            {!(isLogged && walletAddress) ? (
              <UiButton cover onClick={loginMetamask}>
                Connect Wallet
              </UiButton>
              ...
              <UiButton cover onClick={handleChangeNetwork}>
                Switch Network & Purchase
              </UiButton>
            ) : (
              <></>
            )}
            <UiDivider />
            <UiButton cover type="alt" onClick={() => Router.push('/list')}>
              Back To Marketplace
            </UiButton>
          </div>
        </UiLayout>
