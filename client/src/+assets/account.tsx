...
export const Account: NextPage = () => {
  ...
  const renderContent = () => {
    ...
    if (view == 0) {
      ...
      return (
        <AssetsList assets={purchased} disableBatchSelect={true}/>
      )
    } else if (view == 5) {
      return (
        <>
  ...
  return (
    <>
      <UiLayout type="container">
        <UiText wrapper="h1" type="h1" variants={['heading']}>Your Account</UiText>
        <div className={b('row')}>
          <div className={b('columnleft')}>
            <UiText
              className={b('pointer', view === 1 ? ['active'] : [])}
              type="link-caps" variants={['detail']}
              onClick={() => setView(1)}>
              Profile
            </UiText>
            <UiDivider />
