...
export function AssetsList({ assets }: AssetsListProps) {
  ...
  const [selectedAssets, assetToList] = useState<DDO[]>([])

  const addDelAssetToList = (e: any, asset: any) => {
    if (e.target.checked){
      selectedAssets.push(asset)
      assetToList(selectedAssets)
      // console.log("selectedAssets", selectedAssets)
    } else {
      selectedAssets.splice(selectedAssets.indexOf(asset.id),1)
      assetToList(selectedAssets)
      // console.log("selectedAssets", selectedAssets)
    }
  }
      ...
      {assets
        ...
        .map(({ asset, metadata, defi }) => (
          <UiLayout key={asset.id} className={b('asset')}>
            <input id={asset.id + "-checkbox-id"} type="checkbox" onChange={(e: any) => addDelAssetToList(e, asset)} />
            <UiDivider vertical />
